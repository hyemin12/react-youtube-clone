...
개발서버에서 성능테스트를 하면 모든 컴포넌트들이 로드 되어 성능이 낮게 나와 배포페이지에서 테스트를 해보니, 관련된 컴포넌트만 로드되어 성능 값이 변경됨

<img src="./readmeImg/test2.jpg" />

권장사항

- 효율적인 캐시 정책을 사용하여 정적인 애셋 제공하기
  => 영상 썸네일, 채널 썸네일 파일 캐시 수명 길게 설정하라는 진단
- 중요 요청 체이닝 차단
- 요청 수는 낮게, 전송 크기는 작게 유지하기
- 대규모 레이아웃 변경 피하기
  => 데이터를 가져오기 전 회색박스로 레이아웃 먼저 설정하면
  대규모 레이아웃 변경 되지 않으려나?
